...
2. For security reasons change the PORT number that is set to '1234' and save the file.
   For that go to 'package.json' --> 'scripts' --> "devserver": "live-server --port=XXXX --browser=firefox" (choose any number with 4 digits).
   
   NOTE:
      . Default browser: FIREFOX ('--browser=firefox'), this way it is possible to better see the effects of Grids. 
      . To check for RESPONSIVE DESIGN use instead GOOGLE CHROME.
